...
counts <- cdc_state_counts %>% 
  ...
  compute_expected(exclude = exclude_dates, include.trend = FALSE)

kable(counts[1:6,])
```

We use `include.trend = FALSE` because this is less than five years of data.
...
The `compute_expected` function returns the components of the model as the `components` attributed:

```{r}
names(attr(counts, "components"))
```

You can avoid returning these by setting `keep.components` to `FALSE` in the call to `compute_expected`.

You can explore the trend and seasonal component with the `expected_diagnostic` function:
...
cdc_state_counts %>% 
  filter(state == "Massachusetts") %>%
  excess_model(exclude = exclude_dates, intervals = intervals, 
               include.trend = FALSE, verbose = FALSE)
```
...
counts <- collapse_counts_by_age(puerto_rico_counts,
  ...
  summarize(population = sum(population),
            outcome    = sum(outcome), .groups = "drop") %>%
  ungroup()
```
