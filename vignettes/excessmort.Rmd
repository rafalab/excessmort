...
This document is an introduction to excessmort package for analyzing time series count data. The packages was designed to help estimate excess mortality from daily death count data, but can be applied to weekly and monthly data and outcomes other than death.
...
* count tables - Each row repress a date and includes a count and population size.

If you start with record-level data, it is useful to also have a data frame with population sizes fro groups of interest. We expect a population size for each date.
...
The `compute_counts` has a special argument to define agegroups which you can do like this:
...
A first step in most analysis is to estimate the expected count.The `compute_expected` function does this. We do this by assuming the counts $Y_t$ are an overdispresed Poisson random variable with  expected value
\begin{equation}
    \mu_t =N_t \exp[\alpha(t) + s(t) + w(t)]
\end{equation}
with $N_t$ the population at time $t$, $\alpha(t)$ a slow trend to account for the increase in life expectancy we have seen in the last few decades, a seasonal trend $s(t)$ to account for more deaths during the winter, an a day of the week effect $w(t)$.
...
The dispersion parameter is saved as an attribute:
